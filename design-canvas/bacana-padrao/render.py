# -*- coding: utf-8 -*-
"""Achata um .dc.html do padrão Bacana em HTML puro e renderiza no tamanho
exato de publicação (story 1080x1920 ou feed 1080x1350 — a altura é lida do
próprio artboard).

Copiado do padrão das levas (tero-sexta-domingo): estilo e corpo extraídos,
holes resolvidos pelos valores ATUAIS do data-props (onde o editor grava o
slider de véu), e as imagens resolvidas preferindo a foto CHEIA de fotos/
sobre o preview comprimido de img/. O HTML temporário nasce NA PASTA do
.dc.html (armadilha 4.9 do manual: fora dela as imagens não resolvem e a
peça sai só com texto, sem erro nenhum).
"""
...
def achatar(caminho):
    ...
    valores = {
        'veuTopo': str(float(p.get('veuTopo', {}).get('default', 0.45))),
        'veuRodape': str(float(p.get('veuRodape', {}).get('default', 0.60))),
    }
    html = corpo.group(1)
    def resolver(m):
