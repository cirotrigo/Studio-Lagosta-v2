...
AVISOS = []
# Acumulador da LEVA. `AVISOS` e zerado por peca (a sonda do halo roda o
# layout duas vezes e duplicaria tudo); sem um acumulador, so a ULTIMA peca
# conseguia avisar e as demais estouravam em silencio.
TODOS_AVISOS = []
...
def cabe(txt, tam, util=UTIL, ctx=''):
    ...
    if larg > util:
        # O aviso precisa dizer O QUE FAZER, nao so que esta errado: Bevan
        # escala linearmente na largura, entao o maior corpo que cabe e
        # exato. Sem isso a saida era tentativa e erro no olho.
        cabe_em = max(24, int(tam * util / larg))
        AVISOS.append(
            f'  {ctx}"{txt}" mede {larg}px em {tam}px — passa {larg - util}px '
            f'de {util}. Cabe em {cabe_em}px, ou encurte a linha.')
    return larg
...
if __name__ == '__main__':
    ...
    for (chave, pid, nome), bloco in zip(PAG, dados['blocos']):
        ...
        for i, s in enumerate(bloco['slides']):
            ...
            # Guarda os avisos DESTA peca antes que a proxima zere a lista.
    ...
    if TODOS_AVISOS:
        print('\nTITULO QUE NAO CABE (quebra linha sozinho ou passa por baixo da marca):')
        print('\n'.join(TODOS_AVISOS))
