...
async function main() {
  ...
  try {
    ...
    conferir('a medição devolve a FIXAÇÃO inteira (variante + arranjos) e os motivos da provisoriedade', semVarianteMedida.fixacao.variante === semVarianteMedida.variante.id && JSON.stringify(semVarianteMedida.fixacao.arranjos) === JSON.stringify(semVarianteMedida.medicao.arranjos.map((a) => a.id)) && semVarianteMedida.motivosDaProvisoriedade.length > 0, JSON.stringify(semVarianteMedida.fixacao))
    ...
    if (fotoDoAcervo) {
      ...
      conferir('R12: com a foto e a fixação (variante + arranjos) da medição sem foto, a variante e os arranjos são os MESMOS, os arranjos vêm "mantidos" e a escolha deixa de ser provisória', comFotoEFixacao.variante.id === fix.variante && JSON.stringify(comFotoEFixacao.medicao.arranjos.map((a) => a.id)) === JSON.stringify(fix.arranjos) && comFotoEFixacao.medicao.arranjos.every((a) => /mantido/.test(a.motivo) || !/rod[ií]zio/.test(a.motivo)) && comFotoEFixacao.escolhaProvisoria === false, JSON.stringify({ fix, comFoto: comFotoEFixacao.medicao.arranjos.map((a) => [a.id, a.motivo]) }).slice(0, 300))
      console.log(`     (sem a fixação, a medição com a foto escolheu variante ${comFotoSemFixacao.variante.nome} e arranjos ${JSON.stringify(comFotoSemFixacao.medicao.arranjos.map((a) => a.id))} — ${comFotoSemFixacao.variante.id === fix.variante && JSON.stringify(comFotoSemFixacao.medicao.arranjos.map((a) => a.id)) === JSON.stringify(fix.arranjos) ? 'iguais desta vez' : 'DIFERENTES: é o caso de R12'})`)
    } else {
