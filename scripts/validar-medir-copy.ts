...
async function main() {
  ...
  try {
    ...
    conferir('a medição devolve a FIXAÇÃO inteira (variante + arranjos por grupo) e os motivos da provisoriedade', semVarianteMedida.fixacao.variante === semVarianteMedida.variante.id && JSON.stringify(semVarianteMedida.fixacao.arranjos) === JSON.stringify(semVarianteMedida.medicao.arranjos.map((a) => ({ grupo: a.grupo, arranjo: a.id }))) && semVarianteMedida.motivosDaProvisoriedade.length > 0, JSON.stringify(semVarianteMedida.fixacao))

    // ── 3c. R12: a fixação da medição SEM foto reproduz a escolha na medição COM a foto (mesma preparação da composição) ──
    if (fotoDoAcervo) {
      const fix = semVarianteMedida.fixacao
      const comFotoEFixacao = await medirCopyDoProjeto({ projectId: PROJETO, formato: 'story', blocos, nome: 'Prova de fidelidade', tema: 'churrasco', fotoDriveId: fotoDoAcervo, variante: fix.variante, preferencias: { arranjos: fix.arranjos } })
      const comFotoSemFixacao = await medirCopyDoProjeto({ projectId: PROJETO, formato: 'story', blocos, nome: 'Prova de fidelidade', tema: 'churrasco', fotoDriveId: fotoDoAcervo })
      const porGrupo = (m: typeof comFotoEFixacao) => m.medicao.arranjos.map((a) => ({ grupo: a.grupo, arranjo: a.id }))
      conferir('R12/R15: com a foto e a fixação (variante + arranjos POR GRUPO) da medição sem foto, a variante e os arranjos são os MESMOS grupo a grupo, vêm "mantidos" e a escolha deixa de ser provisória', comFotoEFixacao.variante.id === fix.variante && JSON.stringify(porGrupo(comFotoEFixacao)) === JSON.stringify(fix.arranjos) && comFotoEFixacao.medicao.arranjos.every((a) => /mantido/.test(a.motivo) || !/rod[ií]zio/.test(a.motivo)) && comFotoEFixacao.escolhaProvisoria === false, JSON.stringify({ fix, comFoto: comFotoEFixacao.medicao.arranjos.map((a) => [a.grupo, a.id, a.motivo]) }).slice(0, 300))
      console.log(`     (sem a fixação, a medição com a foto escolheu variante ${comFotoSemFixacao.variante.nome} e arranjos ${JSON.stringify(porGrupo(comFotoSemFixacao))} — ${comFotoSemFixacao.variante.id === fix.variante && JSON.stringify(porGrupo(comFotoSemFixacao)) === JSON.stringify(fix.arranjos) ? 'iguais desta vez' : 'DIFERENTES: é o caso de R12'})`)
    } else {
      pendente('R12 com foto real', 'o acervo não devolveu foto com driveFileId — a fixação fica com o teste unitário do serviço')
    }
