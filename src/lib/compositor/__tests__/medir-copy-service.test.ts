...
describe('medirCopyDoProjeto com fotoDriveId — leitura que NÃO publica no Blob (R10)', () => {
  ...
  it('R13: fotoDriveId que NÃO carrega (o Drive falha) não é contexto suficiente — a escolha é PROVISÓRIA, com o motivo e o aviso, e a fixação traz variante e arranjos', async () => {
    mocks.driveMeta.mockRejectedValue(new Error('Drive fora do ar (prova)'))
    mocks.carregarAssinatura.mockResolvedValue(assinaturaMock())
    // outra variante do formato existe (o mock devolve a mesma página para a listagem das outras)
    const r = await medirCopyDoProjeto({ projectId: 6, formato: 'story', fotoDriveId: 'drive-1', blocos: [{ papel: 'headline', linhas: ['Costela'] }] })
    expect(mocks.luz).not.toHaveBeenCalled()
    expect(mocks.carregarAssinatura.mock.calls[0][2]).toMatchObject({ luzDaFoto: null })
  ...
  it('R16: metadados OK mas o DOWNLOAD da miniatura falha (403/conexão): a medição resolve provisória, com aviso e fixação, sem Blob', async () => {
    mocks.driveMeta.mockResolvedValue({ thumbnailLink: 'https://lh3.example/foto=s220' })
    mocks.fetch.mockRejectedValue(new Error('HTTP 403 (prova)'))
    mocks.carregarAssinatura.mockResolvedValue(assinaturaMock())
    const r = await medirCopyDoProjeto({ projectId: 6, formato: 'story', fotoDriveId: 'drive-1', blocos: [{ papel: 'headline', linhas: ['Costela'] }] })
    expect(mocks.luz).not.toHaveBeenCalled()
    expect(r.escolhaProvisoria).toBe(true)
    expect(r.medicao.avisos.some((a) => /Falha ao baixar\/ler a foto/.test(a))).toBe(true)
    expect(r.medicao.avisos.some((a) => /PROVISÓRIA/.test(a))).toBe(true)
    expect(r.fixacao.variante).toBe('p6')
    expect(mocks.put).not.toHaveBeenCalled()
  })
  it('R16: bytes que não são imagem (o sharp não lê): o mesmo — provisória, aviso, nada no Blob', async () => {
    mocks.driveMeta.mockResolvedValue({ thumbnailLink: 'https://lh3.example/foto=s220' })
    mocks.fetch.mockResolvedValue(Buffer.from('<html>Security Checkpoint</html>'))
    mocks.carregarAssinatura.mockResolvedValue(assinaturaMock())
    const r = await medirCopyDoProjeto({ projectId: 6, formato: 'story', fotoDriveId: 'drive-1', blocos: [{ papel: 'headline', linhas: ['Costela'] }] })
    expect(mocks.luz).not.toHaveBeenCalled()
    expect(r.escolhaProvisoria).toBe(true)
  ...
  it('R12: com a foto medida e a variante fixada, mas um arranjo escolhido por rodízio e sem `preferencias.arranjos`, a escolha NÃO é provisória (há luz) e a fixação devolve os arranjos para a composição repetir', async () => {
    ...
    mocks.carregarAssinatura.mockResolvedValue(assinaturaMock())
    mocks.template.mockResolvedValue(null)
    const r = await medirCopyDoProjeto({ projectId: 6, formato: 'story', fotoDriveId: 'drive-1', variante: 'p6', blocos: [{ papel: 'headline', linhas: ['Costela'] }] })
    expect(r.escolhaProvisoria).toBe(false)
