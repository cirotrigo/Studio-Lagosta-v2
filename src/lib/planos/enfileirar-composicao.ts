...
export async function enfileirarComposicaoDoPlanoEm(
  ...
  if (status !== 'reprovado' && anterior && mesmaRevisao && (anterior.status === 'PROCESSING' || (anterior.status === 'COMPLETED' && anterior.resultUrl))) {
    if (jobAnterior) return { generationId: anterior.id, jobId: jobAnterior.id, spec, reaproveitado: true }
  }
  ...
  return { generationId: generation.id, jobId: job.id, spec, reaproveitado: false }
}
