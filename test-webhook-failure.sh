...
# Script para testar o webhook de FALHA do Buffer
# Uso: ./test-webhook-failure.sh [post_id]
...
echo -e "${RED}========================================${NC}"
echo -e "${RED}   Webhook Test - POST FAILURE${NC}"
echo -e "${RED}========================================${NC}\n"
...
echo -e "\n${BLUE}Testando webhook de FALHA para post: ${POST_ID}${NC}\n"

# Payload simulando falha do Buffer
PAYLOAD=$(cat <<EOF
{
  "studio_post_id": "${POST_ID}",
  "buffer_id": "test_buffer_id_$(date +%s)",
  "status": "failed",
  "error_message": "Instagram API error: Media upload failed"
}
EOF
)
...
# Enviar requisição
echo -e "${RED}Enviando requisição de FALHA para webhook...${NC}\n"
...
if [ "$HTTP_CODE" -eq 200 ]; then
  echo -e "\n${GREEN}✅ Webhook de falha processado com sucesso!${NC}"
  echo -e "${GREEN}Post deve estar marcado como FAILED no banco de dados.${NC}\n"
else
